...
typedef struct ExmgMqttPubContext {
    ...
    MQTTClient_createOptions create_opts;
    MQTTClient_nameValue* infos;
...
static void exmg_mqtt_pub_context_init(ExmgMqttPubContext **ptr, ExmgMqttPubConfig config) {
    ...
    s->pub_props = (MQTTProperties) MQTTProperties_initializer;
    s->create_opts = (MQTTClient_createOptions) MQTTClient_createOptions_initializer;
    s->create_opts.MQTTVersion = MQTT_VERSION;
    s->infos = MQTTClient_getVersionInfo();
...
static int exmg_mqtt_pub_connect(ExmgMqttPubContext *s)
{
    ...
    if (rc == MQTTCLIENT_SUCCESS) {
        ...
        av_log(NULL, AV_LOG_ERROR, "MQTTClient_connect failed to: %s\n", s->server_uri);
        s->is_connected = 0;
    }
    return s->is_connected;
}
...
static int exmg_mqtt_pub_send(ExmgMqttPubContext *s, uint8_t* message, int message_length)
{
    ...
    int rc = MQTTClient_publish(client, s->config.topic, message_length, message, 0, 0, NULL);
    if (rc != MQTTCLIENT_SUCCESS)
    {

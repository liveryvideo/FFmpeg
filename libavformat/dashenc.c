...
static int dash_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (pkt->flags & AV_PKT_FLAG_KEY && os->packets_written && (!c->has_video || st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) &&
        ...
        const int64_t curr_seg_duration = av_rescale_q(pkt->pts - os->start_pts, st->time_base, AV_TIME_BASE_Q);
        av_log(s, AV_LOG_INFO, "-----------------Key frame, pts: %" PRId64 ", c->has_video: %d, new_seg_on_keyframe: %d curr_seg_duration: %" PRId64 "\n",
                pkt->pts, c->has_video, c->new_seg_on_keyframe, curr_seg_duration);

        if (curr_seg_duration < os->seg_duration*9/10) {
            av_log(s, AV_LOG_WARNING, "Key frame arrived too early, do not create new segment\n");
        } else {
            c->last_duration = curr_seg_duration;
